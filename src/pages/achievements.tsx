...
export function AchievementsPage() {
  const unlocked = extendedAchievements.filter((a) => a.unlockedAt !== null);
  const locked = extendedAchievements.filter((a) => a.unlockedAt === null);
  const total = extendedAchievements.length;
  const unlockPercent = Math.round((unlocked.length / total) * 100);

  const byRarity = {
    legendary: extendedAchievements.filter((a) => a.rarity === 'legendary'),
    epic: extendedAchievements.filter((a) => a.rarity === 'epic'),
    rare: extendedAchievements.filter((a) => a.rarity === 'rare'),
    common: extendedAchievements.filter((a) => a.rarity === 'common'),
  };
